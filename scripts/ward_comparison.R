...
make_ward_comparison_plot <- function(ward1_df, ward2_df,
                                  ward1, ward2, year) {
    ward1_median <- median(ward1_df$cash_value)
    ward2_median <- median(ward2_df$cash_value)
    
    bind_rows(list(ward_1 = ward1_df,
                   ward_2 = ward2_df),
              .id = "Ward") |>
        ggplot(aes(cash_value)) +
        geom_density(aes(y = ..density.., fill=Ward),
                     alpha = 0.5) +
        geom_vline(xintercept = ward1_median, color="red") + 
        geom_vline(xintercept = ward2_median, color="blue") + 
        scale_x_continuous(labels = label_currency(prefix="$"),
                           n.breaks = 12,
                           limits = c(0, 3000000)) +
        scale_fill_hue(labels = c(ward1, ward2)) +
        theme(axis.text.y = element_blank()) +
        labs(
            title = str_glue("Cash Value Distribution of Ward {ward1} and Ward {ward2} in {year}"),
            x = "Fair Cash Value",
            y = "Number of PINs"
        )
}
...
pin_df <- query_db(year)
ward1_df <- get_ward(pin_df, ward1)
ward2_df <- get_ward(pin_df, ward2)

make_ward_comparison_plot(ward1_df, ward2_df, ward1, ward2, year)
